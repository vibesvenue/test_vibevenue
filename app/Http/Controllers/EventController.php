...
{
    // Fetch all events with their associated category
    $events = Event::with('category')->get(); 

    // Format events to FullCalendar's expected format
    $calendarEvents = $events->map(function ($event) {
        return [
            'title' => $event->name,
            'start' => $event->start_time->format('Y-m-d\TH:i:s'),
            'end' => $event->end_time ? $event->end_time->format('Y-m-d\TH:i:s') : null,
            'description' => $event->description,
            'venue' => $event->venue,
            'price' => $event->ticket_price,
            'id' => $event->id,
            'image' => $event->image,
            'category' => $event->category ? $event->category->name : null,  // Include the category name
        ];
    });
